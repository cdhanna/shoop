...
float noiseLevel = _NoisePower * 8 * worldWind(input.position.xy * .07, float2(3, 3));
			noiseLevel += .1 * worldWind(input.position.xy * .01, float2(2, 2));
// float n = _NoisePower * worldWind(input.position.xy * 4.6, float2(2, 2)) + .7 * _NoisePower * worldWind(input.position.xy * .5, float2(2, 2));
			// c += n * 1.1;
			...
			faceColor = GetColor(sd + noiseLevel, faceColor, outlineColor, outline, softness);
		...
		#if GLOW_ON
			float4 glowColor = GetGlowColor(sd + noiseLevel, scale);
